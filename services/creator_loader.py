...
def load_creator(user_id):
    """
    Loads all necessary data for an AI creator into a single dict:
    - Basic info from users table
    - Behavior (posting_frequency, personality, backstory, online/offline time, etc.)
    - Persona (tone, interests, speaking style, emojis)
    - Looks (eye_color, hair, body_type, etc.)
    - Traits (favorite color, food, etc.)
    - Memory (last 10 chats per fan)
    """
